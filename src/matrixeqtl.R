...
run_summ_stats <- function(data_lst, variants){
  # data_lst: list. output of <init_data>
  # variants: vector of strings. Variant IDs of the variants in a given
  #       region

  genos <- get_genotype(data_lst[['geno_fp']],
                          eids=data_lst[["pheno_eids"]],
                          cols=c('individual', variants))
