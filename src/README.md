...
1. [Scripts](#scripts)
    ...
    1. [plink_raw_to_model_training.py](#plink_raw_to_model_trainingpy)
...
To convert PLINK genotype files into the model training format, we have a script [plink_raw_to_model_training.py](#plink_raw_to_model_trainingpy). The first step to using this script is to use PLINK to recode genotype into their `.traw` format, using a command like this:

```shell script
plink2 \
--recode A-transpose \
--bfile old_plink_geno \
--out new_plink_raw_geno
```
and the python script will operate on the raw text files generated by PLINK. 
...
PredictDB models come with two complementary files, a SQLite database with tables `weights` and `extra`, and a gzipped tab-separated text file where a covariance matrix is stored. 

### SQLite Prediction Models

The prediction models are written into [SQLite](https://www.sqlite.org/index.html) files. Python has a [sqlite3 package](https://docs.python.org/3.8/library/sqlite3.html) which works well for this use case. Pandas can also export DataFrames [to SQL](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.to_sql.html). 

The `weights` table defines the models (with multiple entries per gene), and the `extra` table has one row per gene with model metadata and prediction performance data. Here is an overview of the schema:
...
For a lot of places in the scripts, the terms "gene" and "phenotype" are used interchangably. This is because the library was initially written to create prediction models for gene expression traits. Since then, I've changed the pipeline to train models for other types of traits. So this means there will be the occasional logging message looking like this, when really the script is looping through phenotypes. 
```
INFO --- Working on gene 27 / 27
```

The same is true for the PredictDB model format: the "phenotype" column is actually called "gene". 
...
## plink_raw_to_model_training.py

This script is for transforming PLINK file (or any genotype files PLINK can load) into the model training genotype format. The first step to using this script is to use PLINK to recode genotype into their `.traw` format, using a command like this:
...
--out new_plink_raw_geno
```
and then this python script operates on the raw genotype files created by PLINK. 

| Argument | Description | Example Usage |
| --- | --- | --- |
| -raw_plink | Filepath to raw plink file. Extension should be `.traw` | | 
| -output_prefix | Filename prefix. A file for the dosage and a file for the annotations will be created. | |
| --dosage | Specify if the genotype values should be saved as dosages, rather than integer genotype calls. | |
| --id_mapping | Specify a file for mapping the variant IDs | |
| --whitelist | Specify a file with a whitelist of variants, where only these variants are written into the model training genotype files | |
| --save_bad_keys | If using `--whitelist`, specify this to write an extra file listing the discarded SNPs | |
| -parsimony | Logging amount. 1 means that everything is being logged; 10 (the default) is only high-level messages | |
